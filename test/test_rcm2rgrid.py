...
class Test_rcm2rgrid_float64(ut.TestCase):
    def test_rcm2rgrid_float64(self):
        fo = geocat.ncomp.rcm2rgrid(lat2D, lon2D, fi, lat1D, lon1D)
        np.testing.assert_array_equal(fi, fo[..., ::2, ::2].values)

    def test_rcm2rgrid_msg_float64(self):
        fo = geocat.ncomp.rcm2rgrid(lat2D, lon2D, fi, lat1D, lon1D, msg=fi[0,0,0])
        np.testing.assert_array_equal(fi, fo[..., ::2, ::2].values)

    def test_rcm2rgrid_nan_float64(self):
        fi_np_copy = fi.copy()
        fi_np_copy[:,0,0] = np.nan
        fo = geocat.ncomp.rcm2rgrid(lat2D, lon2D, fi_np_copy, lat1D, lon1D)
...
class Test_rcm2rgrid_float32(ut.TestCase):
    def test_rcm2rgrid_float32(self):
        ...
        fo = geocat.ncomp.rcm2rgrid(lat2D.astype(np.float32), lon2D.astype(np.float32), fi_asfloat32, lat1D.astype(np.float32), lon1D.astype(np.float32))
    ...
    def test_rcm2rgrid_msg_float32(self):
        ...
        fo = geocat.ncomp.rcm2rgrid(lat2D.astype(np.float32), lon2D.astype(np.float32), fi_np_copy, lat1D.astype(np.float32), lon1D.astype(np.float32), msg=fi_np_copy[0,0,0])        
