...
class Test_rgrid2rcm_float64(ut.TestCase):
    def test_rgrid2rcm_float64(self):
        fo = geocat.ncomp.rgrid2rcm(lat1D, lon1D, fi, lat2D, lon2D)
        np.testing.assert_array_equal(fi, fo[..., ::2, ::2].values)

    def test_rgrid2rcm_msg_float64(self):
        fo = geocat.ncomp.rgrid2rcm(lat1D, lon1D, fi, lat2D, lon2D, msg=fi[0,0,0])
        np.testing.assert_array_equal(fi, fo[..., ::2, ::2].values)

    def test_rgrid2rcm_nan_float64(self):
        fi_np_copy = fi.copy()
        fi_np_copy[:,0,0] = np.nan
        fo = geocat.ncomp.rgrid2rcm(lat1D, lon1D, fi_np_copy, lat2D, lon2D)
...
class Test_rgrid2rcm_float32(ut.TestCase):
    def test_rgrid2rcm_float32(self):
        ...
        fo = geocat.ncomp.rgrid2rcm(lat1D.astype(np.float32), lon1D.astype(np.float32), fi_asfloat32, lat2D.astype(np.float32), lon2D.astype(np.float32))
    ...
    def test_rgrid2rcm_msg_float32(self):
        ...
        fo = geocat.ncomp.rgrid2rcm(lat1D.astype(np.float32), lon1D.astype(np.float32), fi_np_copy, lat2D.astype(np.float32), lon2D.astype(np.float32), msg=fi_np_copy[0,0,0])        
